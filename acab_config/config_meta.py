...
class ConfigSingletonMeta(type(Protocol)):
    ...
    def __call__(cls, *paths:str|Path, hooks:None|bool|list[Callable[..., Any]]=None, **kwargs) -> None|Config_i:
        """
        If "build" is passed as a kwarg, build then return the config singleton.
        Duplicate "build" calls error.
        With "reset" as a kwarg, invalidate the singleton instance *without* clearing it.
